...
private:
  ...
  vk::Format findSupportedFormat(const std::vector<vk::Format> &candidates,
    ...
    // Search through each candidate format and return the first one that meets
    // requirements
    auto formatIt = std::ranges::find_if(candidates, [&](auto const format) {
      ...
      // Check whether the requested features are available under the chosen
      // tiling mode
  ...
  void createTextureSampler() {
    ...
    samplerInfo.mipmapMode =
        vk::SamplerMipmapMode::eLinear; // Linear mipmap blending
    samplerInfo.addressModeU =
        vk::SamplerAddressMode::eRepeat; // Wrap texture in U
    samplerInfo.addressModeV =
        vk::SamplerAddressMode::eRepeat; // Wrap texture in V
    samplerInfo.addressModeW =
        vk::SamplerAddressMode::eRepeat;    // Wrap texture in W
    ...
    samplerInfo.borderColor =
        vk::BorderColor::eIntOpaqueBlack; // For clamp modes
    samplerInfo.unnormalizedCoordinates =
        VK_FALSE; // Use normalized texture coordinates
  ...
  void generateMipmaps(vk::raii::Image &image, vk::Format imageFormat,
                       ...
                       uint32_t mipLevels) {
    // Ensure that the image format supports linear filtering (required for
    // blitting)
  ...
  void copyBufferToImage(const vk::raii::Buffer &buffer, vk::raii::Image &image,
    ...
    region.imageExtent =
        vk::Extent3D{width, height, 1}; // Copy entire image area
  ...
  void createDescriptorPool() {
    // Two descriptor pool sizes: one for uniform buffers, one for image
    // samplers
  ...
  void createUniformBuffers() {
    // Clear existing containers in case of re-creation (e.g., swapchain
    // recreation)
  ...
  void createBuffer(vk::DeviceSize size, vk::BufferUsageFlags usage,
    ...
    bufferInfo.sharingMode =
        vk::SharingMode::eExclusive; // Owned by one queue family
   ...
   */
  void drawFrame() {
    // Wait for the current frame’s fence to ensure that the GPU has finished
    ...
    // Define which pipeline stage should wait before executing rendering
    // commands.
    vk::PipelineStageFlags waitDestinationStageMask(
        vk::PipelineStageFlagBits::eColorAttachmentOutput);

    // Setup submission info: which semaphores to wait for, which to signal, and
    // which command buffer to execute.
    vk::SubmitInfo submitInfo;
    ...
    // Prepare presentation info: which image to present and which semaphore to
    // wait on.
  ...
  void transition_image_layout(uint32_t imageIndex, vk::ImageLayout oldLayout,
    ...
    // Wrap the barrier in a dependency info struct to submit it to the command
    // buffer.
    vk::DependencyInfo dependencyInfo;
    dependencyInfo.imageMemoryBarrierCount = 1;
    dependencyInfo.pImageMemoryBarriers = &barrier;

    // Issue the pipeline barrier to ensure correct ordering and layout
    // transition.
  ...
  void recordCommandBuffer(uint32_t imageIndex) {
    ...
    resolveAttachmentInfo.imageLayout =
    ...
    presentBarrier.subresourceRange.aspectMask =
        vk::ImageAspectFlagBits::eColor;
  ...
  void createGraphicsPipeline() {
    ...
    rasterizer.depthClampEnable =
        VK_FALSE; // No clamping beyond near/far planes
    ...
    for (auto const &requiredExtension : requiredExtensions) {
      if (std::none_of(extensionProperties.begin(), extensionProperties.end(),
                       [requiredExtension](auto const &extensionProperty) {
                         // Compare each supported extension with the required
                         // one.
  ...
  void createImageViews() {
    ...
    // Iterate over every image in the swap chain and create an image view for
    // it.
  ...
  void pickLogicalGPU() {
    ...
    // Enable extended dynamic state features (for flexible pipeline state
    // changes).
  ...
  void pickPhysicalGPU() {
    ...
    for (auto const &gpu : gpus) {
      ...
      if (isSuitable) {
        ...
        // Query and store the maximum usable MSAA sample count for
        // anti-aliasing.
  ...
  void recreateSwapChain() {
    ...
    // Wait until GPU finishes any ongoing operations before modifying swapchain
    // resources.
    device.waitIdle();
    cleanupSwapChain();
  ...
  void initWindow() {
    ...
    glfwSetWindowUserPointer(window,
                             this); // Associate window with this object.
    glfwSetFramebufferSizeCallback(
        window, framebufferResizeCallback); // Handle resize events.
  }
  ...
  void initVulkan() {
    ...
    createGraphicsPipeline();    ///< Setup graphics pipeline (shaders, states,
                                 ///< etc.).
