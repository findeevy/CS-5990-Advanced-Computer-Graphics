 ...
 *
 * High-level steps:
 * 1. Query surface capabilities and available formats/present modes from the GPU.
 * 2. Choose a surface format, present mode, and extent (resolution).
 * 3. Build a vk::SwapchainCreateInfoKHR describing how the swap chain should behave.
 * 4. Construct the RAII-wrapped vk::raii::SwapchainKHR object.
 * 5. Retrieve raw swap chain images and create RAII-wrapped image objects so we can
 *    create corresponding image views (one per swap chain image).
 *
 * Important notes:
 * - This function can throw on Vulkan errors (vk::SystemError) when creating the swapchain
 *   or image views. Callers are responsible for handling exceptions appropriately.
 * - RAII objects hold references to the device; do not destroy the device before the swapchain.
 *
...
void SwapChain::create() {
    // Query the current surface capabilities for the given physical GPU and surface.
    // This provides supported min/max image counts, currentTransform, extents, etc.
    ...
    // Using more images can increase GPU/CPU parallelism; many examples use 2 or 3.
    // Here we prefer 3 for extra buffering, but respect the device's maximum.
    ...
    // mailbox is preferred (low latency, no tearing), fallback to FIFO which is guaranteed.
    auto presentMode =
    ...
    // - imageUsage: how we intend to use the images (color attachment for rendering)
    // - imageSharingMode: exclusive is fastest for single-queue families
    // - preTransform: rotation/transform applied by the surface (use currentTransform)
    // - compositeAlpha: how alpha is handled with other windows (opaque by default)
    ...
    // Wrap the raw images in RAII Image objects so we can safely manage their lifetime
    // when creating ImageViews. Note: these RAII images simply provide RAII semantics
    // for the handle, they do not allocate memory — swapchain images are owned by the swapchain.
    ...
    // Create an image view for each swapchain image. Image views describe how shaders
    // will access image data (format, subresource range, etc.). We store RAII-wrapped
    // ImageView objects in swapChainImageViews. Clearing here ensures repeated create()
    // calls (e.g., during recreation) do not leak previous views.
    swapChainImageViews.clear();
    swapChainImageViews.reserve(raiiSwapChainImages.size());
    for (auto &image : raiiSwapChainImages) {
        // Use the utility helper to create a view; this centralizes consistent creation flags.
        swapChainImageViews.emplace_back(
                vkutils::createImageView(device, image, swapChainImageFormat,
                                         vk::ImageAspectFlagBits::eColor, 1));
    }

    // Note: raiiSwapChainImages temporaries will be destroyed at the end of this function,
    // but the ImageView objects created above hold the necessary references because they
    // are created using the device and underlying VkImage handles. Ensure the device
    // and swapChain outlive the image views; otherwise you will have dangling handles.
}

/**
 * @brief Cleans up swap chain resources.
 *
 * Using RAII containers means most cleanup is done by destructors, but we still:
 * - Clear the vector of image views (explicitly destroying the RAII view objects).
 * - Reset the swapChain RAII handle to release the underlying VkSwapchainKHR.
 *
 * This is called before creating a new swap chain or when shutting down the renderer.
 *
 * @see recreate()
 */
void SwapChain::cleanup() {
    // Explicitly destroy image views now (clear invokes destructors of the RAII objects).
    swapChainImageViews.clear();

    // Reset the swapChain RAII wrapper. Assigning nullptr or default-constructed RAII wrapper
    // releases the VkSwapchainKHR resource immediately.
    swapChain = nullptr;
}
 ...
 *
 * Note: More robust implementations may need to wait for the device to become idle
 * or to free other resources (framebuffers, pipelines) that depend on the swapchain.
 *
 * @see create()
 * @see cleanup()
 */
 ...
 *
 * Explanation:
 * - Some window systems (e.g., Android) provide a fixed currentExtent; in that case the
 *   driver requires you to use that exact extent.
 * - If currentExtent has the special value of UINT32_MAX, the surface allows the app
 *   to choose an extent. We fall back to a default (800x600) and clamp it to the
 *   supported min/max extents provided by the surface capabilities.
 *
 * @param capabilities The surface capabilities retrieved from the physical device.
...
SwapChain::chooseSwapExtent(const vk::SurfaceCapabilitiesKHR &capabilities) {
    ...
    if (capabilities.currentExtent.width !=
        ...
        // Fallback default; in a real application you should query the actual window
        // size (e.g., from GLFW) and use that instead of a hard-coded 800x600.
 ...
 *
 * Present modes:
 * - eMailbox: low latency, no tearing (if supported); similar to triple buffering.
 ...
 *
 * @param availablePresentModes List of supported present modes returned by the driver.
 ...
 *
 * If that specific format is not available we return the first available option.
 * In a more advanced implementation you could score formats based on color depth,
 * linear vs sRGB, and device support.
 *
